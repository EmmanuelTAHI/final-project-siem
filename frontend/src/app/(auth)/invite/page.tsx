...
function ErrorBanner({ message }: { message: string }) {
  return (
    <div
      style={{
        display: "flex", alignItems: "center", gap: 8, padding: 12, borderRadius: 9, marginBottom: 14,
        background: "color-mix(in srgb, var(--danger, #EF4444) 10%, transparent)",
        border: "1px solid color-mix(in srgb, var(--danger, #EF4444) 30%, transparent)",
        color: "var(--danger, #EF4444)", fontSize: 12.5,
      }}
    >
      ...
      <div className="card card-glass auth-card" style={{ width: "100%", maxWidth: 420, padding: 36 }}>
        ...
        {done ? (
          <div style={{ fontSize: 13, color: "var(--text-2)", margin: "20px 0", lineHeight: 1.55, display: "flex", alignItems: "flex-start", gap: 10 }}>
            ...
            <span>Compte activé. Redirection vers la connexion…</span>
          </div>
        ) : !token ? (
          <ErrorBanner message="Lien invalide ou incomplet. Demandez à votre administrateur de vous réinviter." />
        ) : (
            ...
            <div className="flex flex-col gap-4">
              ...
              <Button type="submit" disabled={isLoading} className="w-full mt-1">
                {isLoading ? (
                  ...
                  <>Activer mon compte <ChevronRight size={14} /></>
                )}
              </Button>
            </div>
          </form>
